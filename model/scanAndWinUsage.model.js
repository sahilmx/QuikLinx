...
// constructor
const DwanUsage = function (scanandwin) {
  this.vendor_id=scanandwin.vendor_id;
  this.options= scanandwin.options;
  this.created_at= scanandwin.created_at;
  this.updated_at= scanandwin.updated_at;
};


DwanUsage.create = async (newDwanOption, result) => {
  try {
    const res = await masterDb.insert(newDwanOption).into("scanandwin_usage").returning('id');
...
DwanUsage.findById = async (id, result) => {
  try {
    const res = await masterDb.select("*").from("scanandwin_usage").where("vendor_id", id);
    if (res.length) {
      result(null, res[0]);
      return;
    }
...
DwanUsage.get = async (details,email)=>{
  ...
  try {
    const res= await commonDBConnection.select(details).from('scanandwin_usage').where(email);
...
DwanUsage.updateById = async (id, details, result) => {
  ...
  try {
    const res= await masterDb.from("scanandwin_usage").update(details).where('vendor_id',id);
...
DwanUsage.remove = async (id, result) => {
  try {
    const res = await getConnection().from("scanandwin_usage").del().where({ id });
    console.log(res);
...
DwanUsage.removeAll = async (result) => {
  try {
    const res = await getConnection().from("scanandwin_usage").del();
...
DwanUsage.getAllAdmines = async (title, result) => {
  try {
    const res = await getConnection().select("*").from("scanandwin_usage");
    result(null, res);
  } catch (error) {
    result(null, error);
  }
